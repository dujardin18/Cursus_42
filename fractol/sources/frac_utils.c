...
/*   Updated: 2018/09/07 09:45:32 by fherbine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	ft_prints("%d\n", map->imgs[im_idx].frac.win_ofy);
	...
	map->imgs[im_idx].frac.win_ofx = map->wheel_x - (map->imgs[im_idx].frac.frac_x) / 2;
	map->imgs[im_idx].frac.win_ofy = map->wheel_y - (map->imgs[im_idx].frac.frac_y) / 2;
//	ft_prints("%d\n", map->imgs[im_idx].frac.win_ofy);
}
...
static int get_pos_frac(t_fractal *frac, t_container *map, int im_idx)
{
	...
	i = suit_loop(frac);
	if (!(frac->x + frac->win_ofx <= 0 || frac->x + frac->win_ofx >= WIN_X || frac->y + frac->win_ofy <= 0 || frac->y + frac->win_ofy <= 0))
	{
		if (i == frac->imax)
			put_img_pix(map, im_idx, frac->x + frac->win_ofx, frac->y + frac->win_ofy, frac->col / 4);
		else
			put_img_pix(map, im_idx, frac->x + frac->win_ofx, frac->y + frac->win_ofy, i * frac->col / frac->imax);
	}
	else if (frac->x + frac->win_ofx >= WIN_X)
		return (0);
	frac->y += 1.0;
	return (1);
}
